<div class="py-5">
    <div class="container">
        <div class="row">
            <!-- Footer Menu -->
            <div class="col-lg-5 mt-4 mt-lg-0">
                <x-igniter-orange::nav code="footer-menu" />
            </div>

            <!-- Premium Features Quick Links -->
            <div class="col-lg-2 mt-4 mt-lg-0">
                <h6 class="footer-title">PREMIUM FEATURES</h6>
                <ul class="list-unstyled">
                    <li class="mb-2">
                        <a href="{{ url('/account/features#group-orders') }}" class="text-decoration-none text-muted">
                            <i class="fa fa-users me-2"></i>Group Orders
                        </a>
                    </li>
                    <li class="mb-2">
                        <a href="{{ url('/account/features#scheduled-orders') }}" class="text-decoration-none text-muted">
                            <i class="fa fa-clock me-2"></i>Schedule Order
                        </a>
                    </li>
                    <li class="mb-2">
                        <a href="{{ url('/account/features#subscriptions') }}" class="text-decoration-none text-muted">
                            <i class="fa fa-calendar-check me-2"></i>Meal Plans
                        </a>
                    </li>
                    <li class="mb-2">
                        <a href="{{ url('/track-order') }}" class="text-decoration-none text-muted">
                            <i class="fa fa-motorcycle me-2"></i>Track Order
                        </a>
                    </li>
                </ul>
            </div>

            <div class="col-lg-2 mt-4 mt-lg-0">
                <div class="social-bottom">
                    <h6 class="footer-title">FOLLOW US ON</h6>
                    <div class="d-flex gap-2 mt-3">
                        <a href="https://facebook.com/ugaeats" target="_blank" class="social-icon-link">
                            <img src="{{ asset('themes/demo/assets/images/social/icons8-facebook-logo.svg') }}" alt="Facebook" width="40" height="40">
                        </a>
                        <a href="https://x.com/ugaeats" target="_blank" class="social-icon-link">
                            <img src="{{ asset('themes/demo/assets/images/social/icons8-x-logo-100.svg') }}" alt="X" width="40" height="40">
                        </a>
                        <a href="https://tiktok.com/@ugaeats" target="_blank" class="social-icon-link">
                            <img src="{{ asset('themes/demo/assets/images/social/icons8-tiktok-logo.svg') }}" alt="TikTok" width="40" height="40">
                        </a>
                    </div>
                </div>
            </div>

            <div class="col-lg-3 mt-4 mt-lg-0">
                <div id="newsletter-box">
                    <h5 class="mb-4">Subscribe to our newsletter</h5>
                    <livewire:igniter-orange::newsletter-subscribe-form />
                </div>
            </div>
        </div>
    </div>

    <div class="container">
        <div class="row">
            <div class="col">
                <hr class="mb-3">
            </div>
        </div>
    </div>

    <div class="container">
        <div class="row">
            <div class="col p-2">
                &copy; {{ date('Y') }} UgaEats
            </div>
        </div>
    </div>
</div>

<!-- WhatsApp Floating Button -->
<div class="whatsapp-float">
    <a href="https://wa.me/256779081600?text=Hi%20UgaEats!%20I%20would%20like%20to%20order%20food." 
       class="whatsapp-btn" 
       target="_blank" 
       title="Order via WhatsApp">
        <svg viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
            <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/>
        </svg>
    </a>
    <span class="whatsapp-label">Order on WhatsApp</span>
</div>

<!-- AI Chatbot -->
<div class="chatbot-container" id="chatbotContainer">
    <button class="chatbot-toggle" id="chatbotToggle" title="Chat with UgaEats AI">
        <i class="fa fa-comments"></i>
    </button>
</div>
